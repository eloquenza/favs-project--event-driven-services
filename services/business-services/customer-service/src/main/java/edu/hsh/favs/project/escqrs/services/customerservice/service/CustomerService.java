...
public class CustomerService {
	...
	public Mono<Customer> createCustomer(
		...
		return template.insert(customer)
				.doFirst(() -> log.info(String.format("Database transaction is pending " +
						"commit for entity: %s", customer.toString())))
				// Reading the entity from the DB to ensure it has been committed before trying
				// to emit the associated domain event
				.flatMap(committedEntity -> template
						.selectOne(query(where("username").is(committedEntity.getUsername())),
								Customer.class)
						.single()
						.doOnSuccess(e -> log.info(String.format("Reading entity from DB: %s",
								e.toString()))))
				.doOnSuccess(entity -> log.info(String.format("Database transaction successfully " +
						"completed for entity: %s", entity.toString())))
